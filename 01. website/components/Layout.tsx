...
export default function Layout() {
  ...
  const getPageTitle = () => {
    ...
    if (location.pathname.startsWith('/master/population')) return 'Master Population';
          ...
          <div className="mt-8">
            ...
            {masterExpanded && (
              <div className="mt-1 space-y-1">
                ...
                <Link
                  to="/master/population"
                  className={`flex items-center gap-3 px-12 py-2.5 text-sm transition-colors ${
                    isActive('/master/population')
                      ? 'text-[#5B5FC7] font-semibold'
                      : 'text-gray-400 hover:text-white'
                  }`}
                >
                  <div className={`w-1.5 h-1.5 rounded-full ${isActive('/master/population') ? 'bg-[#5B5FC7]' : 'bg-transparent'}`}></div>
