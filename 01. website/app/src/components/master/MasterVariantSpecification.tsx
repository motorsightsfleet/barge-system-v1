...
import { variantSpecificationApi, VariantSpecification, Pagination } from "../../lib/variantSpecificationApi";
...
export default function MasterVariantSpecification() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<VariantSpecification | null>(null);
  ...
  async function fetchSpecs() {
    ...
    setErrorMsg(null);
    try {
      const res = await variantSpecificationApi.list({ query, status, page, pageSize, sortBy, sortDir });
      setSpecs(res.data);
      setPagination(res.pagination);
    } catch (err) {
      setErrorMsg(err instanceof ApiError ? err.message : "Failed to load variant specifications");
    } finally {
      setLoading(false);
    }
  }
  ...
  function handleDeleteClick(spec: VariantSpecification) {
  ...
  async function handleConfirmDelete() {
    ...
    setDeleting(true);
    try {
      await variantSpecificationApi.remove(deleteTarget.id);
      setDeleteModal("success");
    } catch (err) {
      setDeleteError(err instanceof ApiError ? err.message : "Failed to delete variant specification");
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-base font-bold text-gray-900">Variant Specification</h1>
          <div className="flex gap-2">
            <a
              href={variantSpecificationApi.exportUrl()}
            ...
            <Link
              to="/master/population/variant-specification/create"
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search by Variant Specification"
        ...
        {errorMsg && (
          <div className="mx-4 mt-3 bg-rose-50 border border-rose-200 text-rose-700 text-sm font-medium rounded-xl px-4 py-3">{errorMsg}</div>
        )}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#5B5FC7] text-white">
                ...
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">Variant Specification</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">
                  <button onClick={() => toggleSort("capacityVessel")} className="flex items-center gap-1.5 hover:opacity-80">
                    Capacity Vessel
                    <ChevronsUpDown className="w-3.5 h-3.5" />
                  </button>
                </th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">Axle Configuration</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">
                  <button onClick={() => toggleSort("totalWheel")} className="flex items-center gap-1.5 hover:opacity-80">
                    Total Wheel
                    <ChevronsUpDown className="w-3.5 h-3.5" />
                  </button>
                </th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">
                  <button onClick={() => toggleSort("wheelSize")} className="flex items-center gap-1.5 hover:opacity-80">
                    Wheel Size
                    <ChevronsUpDown className="w-3.5 h-3.5" />
                  </button>
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-5 py-10 text-center text-sm text-gray-500">No variant specifications found.</td></tr>
              ) : (
                specs.map((spec, idx) => (
                  <tr key={spec.id} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="px-5 py-3.5 text-sm text-gray-500">{from + idx}</td>
                    <td className="px-5 py-3.5 text-sm font-semibold text-gray-900">{spec.label}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{spec.capacityVessel}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{spec.axleConfiguration}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{spec.totalWheel}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{spec.wheelSize}</td>
                    <td className="px-5 py-3.5 text-sm"><StatusBadge isActive={spec.isActive} /></td>
                    <td className="px-5 py-3.5 text-right relative">
                      <button
                        onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === spec.id ? null : spec.id); }}
                      ...
                      {openMenuId === spec.id && (
                        ...
                        >
                          <Link to={`/master/population/variant-specification/${spec.id}/edit`} className="block px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Edit
                          </Link>
                          <button
      ...
      {deleteModal === "success" && (
        <ActionModal
          variant="success"
          title="Data Variant Specification successfully deleted"
