...
import { unitApi, Unit, Pagination, UNIT_STATUSES } from "../../lib/unitApi";
...
export default function MasterUnit() {
  ...
  async function fetchUnits() {
    ...
    setErrorMsg(null);
    try {
      const res = await unitApi.list({ query, status, siteId, unitStatus, page, pageSize, sortBy, sortDir });
      setUnits(res.data);
      setPagination(res.pagination);
    } catch (err) {
      setErrorMsg(err instanceof ApiError ? err.message : "Failed to load units");
    } finally {
      setLoading(false);
    }
  }
  ...
  }, [query, status, siteId, unitStatus, page, pageSize, sortBy, sortDir]);
  ...
  async function handleConfirmDelete() {
    ...
    setDeleting(true);
    try {
      await unitApi.remove(deleteTarget.id);
      setDeleteModal("success");
    } catch (err) {
      setDeleteError(err instanceof ApiError ? err.message : "Failed to delete unit");
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-base font-bold text-gray-900">Unit</h1>
          <div className="flex gap-2">
            <a
              href={unitApi.exportUrl()}
            ...
            <Link
              to="/master/population/unit/create"
        ...
        <div className="px-4 py-3 border-b border-gray-100 flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Search by Unit Code, Serial Number, or Unit Type"
          ...
          <select
            value={unitStatus}
            onChange={(e) => { setUnitStatus(e.target.value); setPage(1); }}
            className="px-3 py-1.5 border border-gray-300 rounded-lg text-xs font-medium bg-white focus:outline-none focus:ring-2 focus:ring-[#5B5FC7]/30 focus:border-[#5B5FC7]"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#5B5FC7] text-white">
                ...
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">Serial Number</th>
                <th className="px-5 py-3.5 text-xs font-bold uppercase tracking-wider">Unit Status</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan={9} className="px-5 py-10 text-center text-sm text-gray-500">Loading...</td></tr>
              ) : units.length === 0 ? (
                <tr><td colSpan={9} className="px-5 py-10 text-center text-sm text-gray-500">No units found.</td></tr>
              ) : (
                units.map((unit, idx) => (
                  <tr key={unit.id} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm font-semibold text-gray-900">{unit.unitCode}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{unit.variantSpecification.unitType.name}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{unit.site.name}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{formatDate(unit.arriveDate)}</td>
      ...
      {deleteModal === "success" && (
        <ActionModal
          variant="success"
          title="Data Unit successfully deleted"
