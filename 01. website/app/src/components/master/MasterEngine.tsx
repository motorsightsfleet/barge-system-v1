...
import { engineApi, Engine, Brand, Pagination } from "../../lib/engineApi";
...
export default function MasterEngine() {
  ...
  async function fetchEngines() {
    ...
    setErrorMsg(null);
    try {
      const res = await engineApi.list({ query, status, brandId, page, pageSize, sortBy, sortDir });
      setEngines(res.data);
      setPagination(res.pagination);
    } catch (err) {
      setErrorMsg(err instanceof ApiError ? err.message : "Failed to load engines");
    } finally {
      setLoading(false);
    }
  }
  ...
  }, [query, status, brandId, page, pageSize, sortBy, sortDir]);
  ...
  function handleDeleteClick(engine: Engine) {
  ...
  async function handleConfirmDelete() {
    ...
    setDeleting(true);
    try {
      await engineApi.remove(deleteTarget.id);
      setDeleteModal("success");
    } catch (err) {
      setDeleteError(err instanceof ApiError ? err.message : "Failed to delete engine");
    ...
    <div className="p-8 max-w-[1600px] mx-auto space-y-6" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-base font-bold text-gray-900">Engine</h1>
          <div className="flex gap-2">
            <a
              href={engineApi.exportUrl()}
            ...
            <Link
              to="/master/population/engine/create"
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-5 py-10 text-center text-sm text-gray-500">No engines found.</td></tr>
              ) : (
                engines.map((engine, idx) => (
                  <tr key={engine.id} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="px-5 py-3.5 text-sm text-gray-500">{from + idx}</td>
                    <td className="px-5 py-3.5 text-sm font-semibold text-gray-900">{engine.name}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600">{engine.brand.name}</td>
                    <td className="px-5 py-3.5 text-sm"><StatusBadge isActive={engine.isActive} /></td>
                    <td className="px-5 py-3.5 text-right relative">
                      <button
                        onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === engine.id ? null : engine.id); }}
                        className="p-1.5 rounded-lg bg-[#5B5FC7] text-white hover:bg-indigo-700 transition-colors"
                      >
                        <MoreVertical className="w-4 h-4" />
                      </button>
                      {openMenuId === engine.id && (
                        <div
                          onClick={(e) => e.stopPropagation()}
                          className="absolute right-5 top-11 z-10 w-36 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden"
                        >
                          <Link to={`/master/population/engine/${engine.id}/edit`} className="block px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Edit
                          </Link>
                          <button
                            onClick={() => handleDeleteClick(engine)}
