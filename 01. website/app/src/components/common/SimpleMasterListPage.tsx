...
interface SimpleMasterApi {
  ...
  remove: (id: string) => Promise<{ data: null }>;
  exportUrl: () => string;
}
...
export default function SimpleMasterListPage({ title, subtitle, breadcrumb, basePath, api, searchPlaceholder }: SimpleMasterListPageProps) {
  ...
  async function fetchItems() {
    ...
    setErrorMsg(null);
    try {
      const res = await api.list({ query, status, page, pageSize, sortBy, sortDir });
      setItems(res.data);
      setPagination(res.pagination);
    } catch (err) {
      setErrorMsg(err instanceof ApiError ? err.message : "Failed to load data");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      setDeleteError(err instanceof ApiError ? err.message : "Failed to delete data");
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan={4} className="px-5 py-10 text-center text-sm text-gray-500">Loading...</td></tr>
              ) : items.length === 0 ? (
                <tr><td colSpan={4} className="px-5 py-10 text-center text-sm text-gray-500">No data found.</td></tr>
              ) : (
                items.map((item, idx) => (
